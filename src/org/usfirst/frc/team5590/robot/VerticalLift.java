...
public class VerticalLift extends Subsystem {
	...
	private Victor liftLeftController = RobotMap.liftLeftController;
	private Victor liftRightController = RobotMap.liftRightController;
	...
	public int lifter(int position) { //Lifts to height position
		...
		if (curHeight < posHeight) {
			liftLeftController.set(.3);
			liftRightController.set(.3);
			return position;
		} else {
			return -1;
		}
    ...
    public void setHeight(double amount) {
    	liftLeftController.set(amount);
    	liftRightController.set(amount);
    }
